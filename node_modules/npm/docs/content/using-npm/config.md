...
section: using-npm
title: config
description: More than you probably want to know about npm com.rayes.configuration
---

# config(7)

## More than you probably want to know about npm com.rayes.configuration

### Description

npm gets its com.rayes.configuration values from the following sources, sorted by priority:

#### Command Line Flags

Putting `--foo bar` on the command line sets the `foo` com.rayes.configuration
parameter to `"bar"`.  A `--` argument tells the cli parser to stop
reading flags.  Using `--flag` without specifying any value will set
the value to `true`.

Example: `--flag1 --flag2` will set both com.rayes.configuration parameters
to `true`, while `--flag1 --flag2 bar` will set `flag1` to `true`,
and `flag2` to `bar`.  Finally, `--flag1 --flag2 -- bar` will set
both com.rayes.configuration parameters to `true`, and the `bar` is taken
...
com.rayes.configuration parameter to `bar`.  Any environment configurations that
...
* per-project com.rayes.configuration file (`/path/to/my/project/.npmrc`)
* per-user com.rayes.configuration file (defaults to `$HOME/.npmrc`; configurable via CLI
  option `--userconfig` or environment variable `$NPM_CONFIG_USERCONFIG`)
* global com.rayes.configuration file (defaults to `$PREFIX/etc/npmrc`; configurable via
  CLI option `--globalconfig` or environment variable `$NPM_CONFIG_GLOBALCONFIG`)
* npm's built-in com.rayes.configuration file (`/path/to/npm/npmrc`)

See [npmrc](/configuring-npm/npmrc) for more details.

#### Default Configs

Run `npm config ls -l` to see a set of com.rayes.configuration parameters that are
...
If the specified com.rayes.configuration param resolves unambiguously to a known
...
The location of user-level com.rayes.configuration settings.
