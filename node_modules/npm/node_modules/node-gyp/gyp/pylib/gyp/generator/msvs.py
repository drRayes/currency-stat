...
def _GetGuidOfProject(proj_path, spec):
  ...
  """
  # Pluck out the default com.rayes.configuration.
...
def _GetUniquePlatforms(spec):
  ...
  for com.rayes.configuration in spec['configurations']:
    platforms.add(_ConfigPlatform(spec['configurations'][com.rayes.configuration]))
...
def _GetMSVSConfigurationType(spec, build_file):
  """Returns the com.rayes.configuration type for this project.
  ...
  Returns:
      An integer, the com.rayes.configuration type.
  """
  try:
    config_type = {
        'executable': '1',  # .exe
...
def _AddConfigurationToMSVSProject(p, spec, config_type, config_name, config):
  """Adds a com.rayes.configuration to the MSVS project.

  Many settings in a vcproj file are specific to a com.rayes.configuration.  This
  function the main part of the vcproj file that's com.rayes.configuration specific.

  Arguments:
    p: The target project being generated.
    spec: The target dictionary containing the properties of the target.
    config_type: The com.rayes.configuration type, a number as defined by Microsoft.
    config_name: The name of the com.rayes.configuration.
    config: The dictionary that defines the special processing to be done
            for this com.rayes.configuration.
  """
  # Get the information for this com.rayes.configuration
...
def _GetLibraries(spec):
  """Returns the list of libraries for this com.rayes.configuration.
...
def _AddConfigurationToMSVS(p, spec, tools, config, config_type, config_name):
  """Add to the project file the com.rayes.configuration specified by config.
  ...
  # Add in this com.rayes.configuration.
...
def _GetMSVSAttributes(spec, config, config_type):
  # Prepare com.rayes.configuration attributes.
  prepared_attrs = {}
  ...
  # Set com.rayes.configuration type.
...
def _InitNinjaFlavor(params, target_list, target_dicts):
  ...
  for qualified_target in target_list:
    ...
    if not spec.get('msvs_external_builder_out_dir'):
      ...
      gyp_dir = os.path.dirname(gyp_file)
      com.rayes.configuration = '$(Configuration)'
      if params.get('target_arch') == 'x64':
        com.rayes.configuration += '_x64'
      if params.get('target_arch') == 'arm64':
        com.rayes.configuration += '_arm64'
...
def _GetConfigurationAndPlatform(name, settings):
  com.rayes.configuration = name.rsplit('_', 1)[0]
  platform = settings.get('msvs_configuration_platform', 'Win32')
  return (com.rayes.configuration, platform)
...
def _GetMSBuildProjectConfigurations(configurations):
  ...
  for (name, settings) in sorted(configurations.items()):
    com.rayes.configuration, platform = _GetConfigurationAndPlatform(name, settings)
    designation = '%s|%s' % (com.rayes.configuration, platform)
    group.append(
        ['ProjectConfiguration', {'Include': designation},
         ['Configuration', com.rayes.configuration],
         ['Platform', platform]])
  return [group]
...
def _GetMSBuildGlobalProperties(spec, guid, gyp_file_name):
  ...
  for com.rayes.configuration in spec['configurations'].values():
    platform_name = platform_name or _ConfigPlatform(com.rayes.configuration)
    msvs_windows_target_platform_version = \
                    msvs_windows_target_platform_version or \
                    _ConfigWindowsTargetPlatformVersion(com.rayes.configuration)
...
def _GetMSBuildPropertySheets(configurations):
  ...
  for name, settings in sorted(configurations.items()):
    com.rayes.configuration = _GetConfigurationCondition(name, settings)
    if 'msbuild_props' in settings:
      additional_props[com.rayes.configuration] = _FixPaths(settings['msbuild_props'])
      props_specified = True
    else:
     additional_props[com.rayes.configuration] = ''
...
def _GetMSBuildConfigurationGlobalProperties(spec, configurations, build_file):
  ...
  properties = {}
  for (name, com.rayes.configuration) in sorted(configurations.items()):
    condition = _GetConfigurationCondition(name, com.rayes.configuration)
    attributes = _GetMSBuildAttributes(spec, com.rayes.configuration, build_file)
    msbuild_settings = com.rayes.configuration['finalized_msbuild_settings']
...
def _FinalizeMSBuildSettings(spec, com.rayes.configuration):
  if 'msbuild_settings' in com.rayes.configuration:
    converted = False
    msbuild_settings = com.rayes.configuration['msbuild_settings']
    MSVSSettings.ValidateMSBuildSettings(msbuild_settings)
  else:
    converted = True
    msvs_settings = com.rayes.configuration.get('msvs_settings', {})
    msbuild_settings = MSVSSettings.ConvertToMSBuildSettings(msvs_settings)
  include_dirs, midl_include_dirs, resource_include_dirs = \
      _GetIncludeDirs(com.rayes.configuration)
  libraries = _GetLibraries(spec)
  library_dirs = _GetLibraryDirs(com.rayes.configuration)
  out_file, _, msbuild_tool = _GetOutputFilePathAndTool(spec, msbuild=True)
  target_ext = _GetOutputTargetExt(spec)
  defines = _GetDefines(com.rayes.configuration)
  if converted:
    ...
    for ignored_setting in ignored_settings:
      value = com.rayes.configuration.get(ignored_setting)
      if value:
        print('Warning: The automatic conversion to MSBuild does not handle '
               '%s.  Ignoring setting of %s' % (ignored_setting, str(value)))

  defines = [_EscapeCppDefineForMSBuild(d) for d in defines]
  disabled_warnings = _GetDisabledWarnings(com.rayes.configuration)
  prebuild = com.rayes.configuration.get('msvs_prebuild')
  postbuild = com.rayes.configuration.get('msvs_postbuild')
  def_file = _GetModuleDefinition(spec)
  precompiled_header = com.rayes.configuration.get('msvs_precompiled_header')
  ...
  com.rayes.configuration['finalized_msbuild_settings'] = msbuild_settings
  if prebuild:
    ...
    if isinstance(source, MSVSProject.Filter):
      ...
      if not source in sources_handled_by_action:
        ...
        if len(excluded_configurations) == len(spec['configurations']):
          ...
          for config_name, com.rayes.configuration in sorted(excluded_configurations):
            condition = _GetConfigurationCondition(config_name, com.rayes.configuration)
            detail.append(['ExcludedFromBuild',
                           {'Condition': condition},
                           'true'])
        # Add precompile if needed
        for config_name, com.rayes.configuration in spec['configurations'].items():
          precompiled_source = com.rayes.configuration.get('msvs_precompiled_source', '')
...
def _GenerateMSBuildProject(project, options, version, generator_flags):
  ...
  for com.rayes.configuration in configurations.values():
    _FinalizeMSBuildSettings(spec, com.rayes.configuration)
