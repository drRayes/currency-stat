...
# project-specific and com.rayes.configuration-specific.  The normal choice,
# DERIVED_FILE_DIR, is target-specific, which is thought to be too restrictive
# as it is likely that multiple targets within a single project file will want
# to access the same set of generated files.  The other option,
# PROJECT_DERIVED_FILE_DIR, is unsuitable because while it is project-specific,
# it is not com.rayes.configuration-specific.  INTERMEDIATE_DIR is defined as
...
class XcodeProject(object):
  ...
  def Finalize1(self, xcode_targets, serialize_all_tests):
    # Collect a list of all of the build com.rayes.configuration names used by the
    # various targets in the file.  It is very heavily advised to keep each
    # target in an entire project (even across multiple project files) using
    # the same set of com.rayes.configuration names.
    configurations = []
    ...
    # a new one specifying all of the com.rayes.configuration names used by the various
    # targets.
    try:
    ...
    #
    # Note that for com.rayes.configuration-relative files such as anything relative to
    # _intermediate_var, for the purposes of UI tree view display, Xcode will
    # only resolve the com.rayes.configuration name once, when the project file is
    # opened.  If the active build com.rayes.configuration is changed, the project file
    ...
    # per-com.rayes.configuration values are applied.
...
def PerformBuild(data, configurations, params):
  ...
  for config in configurations:
    ...
    arguments += ['-com.rayes.configuration', config]
...
""" % makefile_name
    ...
    for configuration_name in configuration_names:
      com.rayes.configuration = spec['configurations'][configuration_name]
      xcbc = xct.ConfigurationNamed(configuration_name)
      for include_dir in com.rayes.configuration.get('mac_framework_dirs', []):
        xcbc.AppendBuildSetting('FRAMEWORK_SEARCH_PATHS', include_dir)
      for include_dir in com.rayes.configuration.get('include_dirs', []):
        xcbc.AppendBuildSetting('HEADER_SEARCH_PATHS', include_dir)
      for library_dir in com.rayes.configuration.get('library_dirs', []):
        if library_dir not in xcode_standard_library_dirs and (
            not xcbc.HasBuildSetting(_library_search_paths_var) or
            library_dir not in xcbc.GetBuildSetting(_library_search_paths_var)):
          xcbc.AppendBuildSetting(_library_search_paths_var, library_dir)

      if 'defines' in com.rayes.configuration:
        for define in com.rayes.configuration['defines']:
          set_define = EscapeXcodeDefine(define)
          xcbc.AppendBuildSetting('GCC_PREPROCESSOR_DEFINITIONS', set_define)
      if 'xcode_settings' in com.rayes.configuration:
        for xck, xcv in com.rayes.configuration['xcode_settings'].items():
          xcbc.SetBuildSetting(xck, xcv)
      if 'xcode_config_file' in com.rayes.configuration:
        config_ref = pbxp.AddOrGetFileInRootGroup(
            com.rayes.configuration['xcode_config_file'])
